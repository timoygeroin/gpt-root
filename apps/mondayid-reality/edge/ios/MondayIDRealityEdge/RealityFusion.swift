...
public enum AuthorityState: String, Codable, Sendable {
    ...
    case retainedActiveDuringCoverageGap = "RETAINED_ACTIVE_DURING_COVERAGE_GAP"
...
public enum RealityFusion {
    public static func derive(official: OrefObservation, profile: PersonalProfile) -> LocalReality {
        ...
        if relevant.isEmpty {
            return LocalReality(
                state: .officialObservedNoActiveItems,
                relevantAlerts: [],
                affectedContexts: [],
        ...
        return LocalReality(
            ...
            relevantAlerts: relevant,
            affectedContexts: contexts.sorted(),
            action: "FOLLOW_CURRENT_OFFICIAL_INSTRUCTIONS",
            reason: "An official-origin active item intersects a saved personal context."
        )
    }

    static func retainActive(_ prior: LocalReality) -> LocalReality {
        LocalReality(
            state: .retainedActiveDuringCoverageGap,
            relevantAlerts: prior.relevantAlerts,
            affectedContexts: prior.affectedContexts,
            action: "VERIFY_CURRENT_OFFICIAL_CHANNEL",
            reason: "Foreground coverage was lost after an active official-origin observation. The last active state is retained until an authoritative observation clears or changes it."
        )
    }
