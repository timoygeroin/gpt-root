...
final class ProfileStore: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        self.home = defaults.string(forKey: "reality.profile.home") ?? ""
        self.work = defaults.string(forKey: "reality.profile.work") ?? ""
        self.family = defaults.string(forKey: "reality.profile.family") ?? ""
        self.route = defaults.string(forKey: "reality.profile.route") ?? ""
        self.hasCompletedSetup = defaults.bool(forKey: "reality.profile.setup")
        self.isLoading = false
    }

    var profile: PersonalProfile {
        PersonalProfile(
            homeAreas: parse(home),
            workAreas: parse(work),
            familyAreas: parse(family),
            routeAreas: parse(route)
        )
    }
